...
public class PrimaryController implements Initializable {
    ...
    public void addQuestionHandler(ActionEvent evt) {
        Question q = new Question(UUID.randomUUID().toString(), txtContent.getText(), cbCategories.getSelectionModel().getSelectedItem().getId());
        
        List<Choice> choices = new ArrayList<>();
        choices.add(new Choice(UUID.randomUUID().toString(), txtA.getText(), rdoA.isSelected(), q.getId()));
        choices.add(new Choice(UUID.randomUUID().toString(), txtB.getText(), rdoB.isSelected(), q.getId()));
        choices.add(new Choice(UUID.randomUUID().toString(), txtC.getText(), rdoC.isSelected(), q.getId()));
        choices.add(new Choice(UUID.randomUUID().toString(), txtD.getText(), rdoD.isSelected(), q.getId()));
        ...
        try {
            if (s.addQuestion(q, choices) == true)
                ...
                Utils.getBox("ADD FAILED", Alert.AlertType.ERROR).show();
        } catch (SQLException ex) {
            Utils.getBox("ADD FAILED", Alert.AlertType.ERROR).show();
            Logger.getLogger(PrimaryController.class.getName()).log(Level.SEVERE, null, ex);
        }
